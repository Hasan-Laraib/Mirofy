...
const tmp = fs.mkdtempSync(path.join(os.tmpdir(), 'mirofy-gallery-'));
...
test('generated proof gallery matches its sources, receipts, and checked-in artifacts', () => {
  ...
  assert.equal(manifest.mirofyVersion, JSON.parse(fs.readFileSync(path.join(skillRoot, 'package.json'))).version);
  ...
  assert.match(html, /href="https:\/\/github\.com\/Hasan-Laraib\/Mirofy\/issues\/new\?template=showcase\.yml"[^>]+rel="noopener noreferrer"/);
