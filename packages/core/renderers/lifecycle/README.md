...
Render `diagram_type: "lifecycle"` JSON files into the standard Mirofy HTML
template.

```bash
node mirofy/renderers/lifecycle/render-lifecycle.mjs input.lifecycle.json output.html
```

The renderer validates input against `mirofy/schemas/lifecycle.schema.json`
...
`mirofy/examples/agent-run.lifecycle.json`.

The schema lives at:

```text
mirofy/schemas/lifecycle.schema.json
```
