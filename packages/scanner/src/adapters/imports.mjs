...
// The walk and the ignore rules live in ../files.mjs so that this adapter and
// the coverage denominator cannot disagree about what files exist. They did:
// the scan counted only what adapters had examined, so a repository this
// adapter cannot read reported a denominator of zero rather than a list of
// files nothing looked at.
function* walk(root) {
  for (const file of repositoryFiles(root)) {
    if (SOURCE_EXTENSIONS.includes(path.extname(file))) yield file;
  }
}
